### FUNCTION TO COMPUTE GENERATE YEARLY MEAN DAILY RAINFALL WITH FOREST LOSS ### 
...
plotPcp <- function (pred.table, SW) {
  ...
  mo <- by.month.d[,1]
  dry.mo <- as.vector(t(mo[1:3]))
  wet.mo <- as.vector(t(mo[10:12]))
  
  driest <- filter(by.year.pivot, mo %in% dry.mo)
  driest$id <- 1
  driest.mo <- aggregate(driest[1:3,], list(driest$id), mean)
  ...
  plot(plot)
  ggsave(plot=plot, filename=paste0('D:/THESIS_PP/final results1/',  Sys.Date(),'_', SW, '_pcp_dry.png'), device='png', dpi=100, width = 11, height = 8, units='in')
  
  wettest <- filter(by.year.pivot, mo %in% wet.mo)
  wettest$id <- 1
  wettest.mo <- aggregate(wettest[1:3,], list(wettest$id), mean)
